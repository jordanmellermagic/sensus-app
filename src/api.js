const API_BASE =
  import.meta.env.VITE_API_BASE || "https://sensus-api-wd2o.onrender.com";
...
export async function fetchDataPeek(userId) {
  const res = await fetch(
    `${API_BASE}/data_peek/${encodeURIComponent(userId)}`
  );
...
export async function clearDataPeek(userId) {
  const res = await fetch(
    `${API_BASE}/data_peek/${encodeURIComponent(userId)}/clear`,
    { method: "POST" }
  );
  return handle(res);
}
...
export async function resetAll(userId) {
  const res = await fetch(
    `${API_BASE}/clear_all/${encodeURIComponent(userId)}`,
    { method: "POST" }
  );
  return handle(res);
}
...
export async function fetchNotePeek(userId) {
  const res = await fetch(
    `${API_BASE}/note_peek/${encodeURIComponent(userId)}`
  );
  if (res.status === 404) return null;
  return handle(res);
}
...
export async function fetchScreenPeek(userId) {
  const res = await fetch(
    `${API_BASE}/screen_peek/${encodeURIComponent(userId)}`
  );
  if (res.status === 404) return null;
  return handle(res);
}
...
export async function fetchScreenshotBlob(userId) {
  const res = await fetch(
    `${API_BASE}/screen_peek/${encodeURIComponent(userId)}/screenshot?${Date.now()}`
  );
...
export async function setCommand(userId, command) {
  const res = await fetch(
    `${API_BASE}/commands/${encodeURIComponent(userId)}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
